...
//			scale - 
//-----------------------------------------------------------------------------
void FX_BMAirboatGunImpact( const Vector &origin, const Vector &normal, float scale )
{
	// Normal metal spark
	FX_MetalSpark( origin, normal, normal, (int) scale );
...
// Purpose: Handle weapon impacts from the airboat gun shooting (cheaper versions)
//-----------------------------------------------------------------------------
void ImpactBMAirboatGunCallback( const CEffectData &data )
{
	VPROF_BUDGET( "ImpactBMAirboatGunCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING );
	...
	if ( Impact( vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr, IMPACT_NODECAL | IMPACT_REPORT_RAGDOLL_IMPACTS ) )
	{
		FX_BMAirboatGunImpact( vecOrigin, tr.plane.normal, 2 );
	}
#else
	FX_BMAirboatGunImpact( vecOrigin, tr.plane.normal, 1 );
#endif
}

DECLARE_CLIENT_EFFECT( "BMAirboatGunImpact", ImpactBMAirboatGunCallback );


//-----------------------------------------------------------------------------
// Purpose: Handle weapon impacts from the helicopter shooting (cheaper versions)
//-----------------------------------------------------------------------------
void ImpactApacheCallback( const CEffectData &data )
{
	VPROF_BUDGET( "ImpactApacheCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING );
	...
	if ( Impact( vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr, IMPACT_NODECAL | IMPACT_REPORT_RAGDOLL_IMPACTS ) )
	{
		FX_BMAirboatGunImpact( vecOrigin, tr.plane.normal, IsXbox() ? 1 : 2 );
...
DECLARE_CLIENT_EFFECT( "ApacheImpact", ImpactApacheCallback );
#endif
