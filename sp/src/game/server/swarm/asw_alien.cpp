...
ConVar asw_alien_sense_dist_swarm_sense( "asw_alien_sense_dist_swarm_sense", "576.0" );
ConVar asw_alien_sense_dist_look( "asw_alien_sense_dist_look", "768.0" );
ConVar asw_alien_sense_dist_too_far( "asw_alien_sense_dist_too_far", "1500.0" );
ConVar asw_alien_sense_long_dist_swarm_sense( "asw_alien_sense_long_dist_swarm_sense", "1152.0" );
ConVar asw_alien_sense_long_dist_look( "asw_alien_sense_long_dist_look", "1536.0" );
ConVar asw_alien_sense_long_dist_too_far( "asw_alien_sense_long_dist_too_far", "2000.0" );
#else
...
ConVar asw_alien_speed_scale_easy("asw_alien_speed_scale_easy", "1");
ConVar asw_alien_speed_scale_normal("asw_alien_speed_scale_normal", "1");
ConVar asw_alien_speed_scale_hard("asw_alien_speed_scale_hard", "1");
ConVar asw_alien_speed_scale_insane("asw_alien_speed_scale_insane", "1");
...
void CASW_Alien::NPCInit()
{
	...
	m_flDistTooFar = asw_alien_sense_dist_too_far.GetFloat();	// seems this is used as an early out for checking attack conditions or not, also for LOS?

	// if flagged, alien can see twice as far
	if ( HasSpawnFlags( SF_NPC_LONG_RANGE ) )
	{		
		SetDistSwarmSense( asw_alien_sense_long_dist_swarm_sense.GetFloat() );
		SetDistLook( asw_alien_sense_long_dist_look.GetFloat() );
		m_flDistTooFar = asw_alien_sense_long_dist_too_far.GetFloat();
	}
#else
	// set default alien swarm sight/sense distances
...
void CASW_Alien::SetupPushawayVector()
{
	...
	for ( int i = 0; i < gpGlobals->maxClients; i++ )
	{
		...
		Vector diff = m_vecLastPushAwayOrigin - pPlayer->GetAbsOrigin();
		float dist = diff.Length();
		VectorNormalize( diff );
...
AI_BEGIN_CUSTOM_NPC( asw_alien, CASW_Alien )
	...
	DECLARE_ACTIVITY( ACT_SPINAROUND )
#endif
